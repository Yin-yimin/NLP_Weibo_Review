...
class Score(object):
        ...
        def load_sentiment_dict(self, dict_path):
            ...
            with open(dict_path, 'r', encoding='UTF-8') as f:
                for index, line in enumerate(f):
                    ...
                    # 转换情感倾向的表现形式, 负数为消极, 0为中性, 正数为积极
                    # 数值*绝对值大小*表示极性的强度！ // 分成3类，极性：褒(+1)、中(0)、贬(-1)
                    value = None
                    if polar == 0:            # neutral
                        value = 0
        ...
        def get2score_position(self, words):
            sen_word, not_word, degree_word =  self.classify_words(words)   # 情感词、否定词、程度副词
...
if __name__ == '__main__':
    sentiment_dict_path = "dictionary/sentiment_words_chinese.tsv" 
    degree_dict_path = "dictionary/degree_dict.txt"
    stop_dict_path = "dictionary/stop_words.txt"

    #文件读取
    data = pd.read_excel('data.xlsx')

    #文件写入
    c = open("output/emotion_score.csv", "a+", newline='', encoding='utf_8_sig')
    writer = csv.writer(c)
    writer.writerow(["texts","words","score"])

    #分句功能 否定词程度词位置判断
    score = Score(sentiment_dict_path, degree_dict_path, stop_dict_path )

    for temp in data['texts']:
        tlist = []
        words = [x for x in jieba.cut(str(temp))] #分词
        #print(words)     
